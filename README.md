...
- **Settlement is a number with a source.** Pyth publishes the price, the rule
  ...
  band. No vote, no committee, no dispute window. If that update is late or
  unsure, the round voids and refunds, depositors first.
- **Bands the anchor sets.** Each coin's series learns its anchor's volatility
  on chain from Pyth closes, and a round's band width is set from it when the
  round opens. A day can be funded up to 31 days ahead.
...
feeds; the app names the stand-in only on a round's page and the fund sheet,
with a note. A series learns its volatility from Pyth closes and opens rounds
once it has 20 daily returns; funding a day never waits for that. The keeper is
