...
export function Ticket(p: Props) {
  ...
  return (
    <section className="panel ticket">
      ...
      {tab === "house" ? <LpPanel refs={p.refs} ladder={p.ladder} quoteSymbol={p.quoteSymbol} now={p.now} transferFee={p.transferFee} bare /> : p.final ? <Collect {...p} /> : p.selected ? <Sell {...p} pos={p.selected} /> : <Buy {...p} />}
    </section>
  );
}

// ── buy a new line ───────────────────────────────────────────────────────────
function Buy(p: Props) {
  ...
  const submit = () => { if (!q || !s || !shares || !publicKey) return; send.mutate({ computeUnits: stook.tradeComputeUnits(s), ixs: [ensureAta(l.quoteMint, publicKey, p.refs.tokenProgram), stook.tradeLadderIx(p.refs, { user: publicKey, userToken: ataOf(l.quoteMint, publicKey, p.refs.tokenProgram), shape: s, shares, limit: (q.total * 1005n) / 1000n })] }); };
  ...
  return (
    <>
      ...
      {!s ? <p className="explain">{p.tradeable ? (p.mode === "line" ? "Click the price you expect at the close." : "Drag across the range you expect.") : l.status === "seeding" ? "Opening in a moment — the keeper is posting the opening price. Deposits are open." : "Trading is closed; the bell is next."} {p.positions.length > 0 && <>Click one of your lines on the chart to sell it.</>}</p>
        : <div className="shape-desc">{p.symbol} at {where}{existing && <span className="muted"> · adds to your {fmtAmount(existing.position.shares, dec)} sh here</span>}</div>}
      {s && (
        <table className="ladder-table">
          <thead><tr><th>If it lands</th><th>chance</th><th>you get back</th><th>on stake</th></tr></thead>
          <tbody>
            {odds.map(([lv, pr]) => { const back = shares ? shares * BigInt(lv) : 0n, x = q && q.total > 0n ? Number(back) / Number(q.total) : null; return <tr key={lv}><td>{s.h === 1 ? "inside" : lv === s.h ? "on your band" : `${s.h - lv} off`}</td><td className="mono">{(Number(pr) / 1e16).toFixed(1)}%</td><td className="mono">{fmtAmount(back, dec)}</td><td className={`mono ${x !== null && x < 1 ? "down" : "amber"}`}>{x !== null ? `${x.toFixed(2)}×` : ""}</td></tr>; })}
      ...
      {q && pays !== null && <dl className="quote"><div><dt>You pay</dt><dd className="mono">{fmtAmount(pays, dec)} {p.quoteSymbol}</dd></div>{pays !== q.total && <div><dt>of which the coin's transfer fee</dt><dd className="mono">{fmtAmount(pays - q.total, dec)}</dd></div>}<div><dt>best case</dt><dd className="mono amber">{fmtAmount(q.maxPayout, dec)} ({(Number(q.maxPayout) / Number(q.total)).toFixed(1)}×)</dd></div></dl>}
      {short && <p className="warn">You hold {fmtAmount(balance.data!, dec)} {p.quoteSymbol}; this costs {fmtAmount(pays!, dec)}.</p>}
      <button className="primary" disabled={!q || !p.tradeable || send.isPending || !publicKey || short} onClick={submit}>{!publicKey ? "Connect a wallet" : !p.tradeable ? "Not trading" : !s ? "Draw a line first" : send.isPending ? "Sending…" : `Buy ${text} shares`}</button>
    </>
  );
}
...
function Sell(p: Props & { pos: PositionRow }) {
  ...
  const submit = () => { if (!q || !publicKey) return; send.mutate({ computeUnits: stook.tradeComputeUnits(s), ixs: [ensureAta(l.quoteMint, publicKey, p.refs.tokenProgram), stook.tradeLadderIx(p.refs, { user: publicKey, userToken: ataOf(l.quoteMint, publicKey, p.refs.tokenProgram), shape: s, shares: -size, limit: (q.total * 995n) / 1000n })] }, { onSuccess: p.onDeselect }); };
  return (
    <>
      ...
      {q && get !== null && <dl className="quote"><div><dt>You receive</dt><dd className="mono">{fmtAmount(get, dec)} {p.quoteSymbol}</dd></div><div><dt>you paid for these</dt><dd className="mono">{fmtAmount(paidFor, dec)}</dd></div><div><dt>result</dt><dd className={`mono ${get >= paidFor ? "up" : "down"}`}>{get >= paidFor ? "+" : "−"}{fmtAmount(get >= paidFor ? get - paidFor : paidFor - get, dec)}</dd></div></dl>}
...
function Collect(p: Props) {
  ...
  const owed = p.positions.map((r) => ({ r, amount: l.status === "settled" && l.settledBin !== null ? r.position.shares * BigInt(stook.level(r.position.shape, l.settledBin)) : l.status === "void" ? r.position.netPaid : 0n }));
  const lp = p.tranches.map((t) => { const k = l.settledBin; const v = l.status === "settled" && k !== null ? stook.tranchePrincipal(t.tranche.deposit, stook.tranchePnl(t.tranche.b, t.tranche.join.w[k]!, t.tranche.join.sum, l.curve.w[k]!, l.curve.sum), dec) + stook.trancheFees(t.tranche.b, dec, l.accFee, t.tranche.feeSnap) : t.tranche.deposit; return { t, v }; });
  ...
  return (
    <>
      <p className="explain">{l.status === "void" ? "The round was void: every line and deposit comes back as paid." : `The bell rang. Band ${l.settledBin} landed.`}</p>
