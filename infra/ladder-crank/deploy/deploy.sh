...
pkill -f "$HOME/stook/infra/ladder-crank/src/index.mjs --watch" || true   # not a bare "index.mjs --watch": other services on the box match that
pkill -f "infra/tape/src/index.mjs" || true
~/ladder-crank-keepalive.sh
~/stook-tape-keepalive.sh
sleep 3
echo "keepers running: $(pgrep -fc "$HOME/stook/infra/ladder-crank/src/index.mjs --watch")"; echo "tape running: $(pgrep -fc "infra/tape/src/index.mjs")"; tail -4 ~/stook-tape.log
