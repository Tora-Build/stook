...
- "$STOOK is live on StonkFun, anchored to SPY": true (mint relaunched 2026-09-24 as a standard token; the first mint `GWrd84X5…` is retired).
...
- "Rounds quoted in $STOOK": the mint `` is a standard Token-2022 coin with no transfer fee. The program handles fee-bearing mints (deposits credited by what arrives; proven against the real mint bytes). On mainnet the protocol authority must `approve_quote_mint` it once.
